...
def combine_bert(data, data_path):
    data_vector = load_data(data_path)
...
def combine_with_result(result, data):
    for key in result.keys():
        ...
        result[key]['new_name'] = re.sub(r'\([0-9]+/([0-9]+)\s*\)', '', result[key]['name'])
    return result
...
def sim(result_with_vector : dict, label_sim : dict):
    ...
    for label in [x[0] for x in label_sim]:
        ...
        for key in result_with_vector.keys():
            if int(result_with_vector[key]['label']) == int(label):
                if flag : 
    ...
    for sim_key in sim_dict.keys():
        ...
        for sim in sim_dict[sim_key]:
            for key in result_with_vector.keys():
                if result_with_vector[key]['similarity'] == sim and int(result_with_vector[key]['label']) == int(sim_key):
                    count += 1
    ...
    for_store = for_store[['year', 'name' , 'label', 'year_start', 'year_end', 'keyword', 'ner', 'tf_idf', 'description', 'order', 'department']]
    for_store.to_csv(f'{nar.util.XML_PATH}/final_{len(label_sim)}.csv', index = False)
    print(for_store)
    return sim_dict
...
if __name__ =="__main__":
    ...
    df = df[['計畫完整中文名稱']]
    df = df.to_dict('index')
    data = combine_bert(df, dataset_path)
    combined_result = pd.read_csv(f'{nar.util.XML_PATH}/combined_result.csv')
    ...
    # 需要跟著 label 數量更改
    label_similarity = label_sim(combined_result, 50)
